...
` i != end && printf(" ");` 空格格式，`!i ||printf(" ");`    少用`if else`分支判断

`if（is_rev && is_prime)`  比 `is_prime && is_rev`   更好，rev更少
...
```cpp
inline int digit(int n);    //内联函数，经常使用，提前加载;不一定成功，当代码空间，逻辑过于复杂
static int arr[];     //静态数组，一次使用就始终存在
```
...
![image-20200613153142100](file:///home/worst/Nutstore%20Files/%E6%88%91%E7%9A%84%E5%9D%9A%E6%9E%9C%E4%BA%91/Dynamic/6.%E6%97%A5%E5%B8%B8/%E4%B8%B4%E6%97%B6.assets/image-20200613153142100.png?lastModify=1615210928)
